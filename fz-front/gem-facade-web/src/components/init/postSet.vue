  ...
  <div class="postSet">
    ...
    <div class="set_content">
      <div class="post_tree_area dragscroll">
        <div class="post_tree ">
          <family-tree :data="departmentData" @addNode="operateModal" @editNode="operateModal"></family-tree>
          <p class="no_data_tip" v-if="departmentData.length===0">缺少第一级部门数据,请返回上一步重新导入数据</p>
        </div>
      </div>
      <div class="btns clearfix" style="width: 320px;margin: 40px auto">
        <div class="btn btn-light fl" @click="childFun('2-1','1-1','prev')">上一步</div>
        <div class="btn btn-primary fr" @click="childFun('2-1','2-2','next')">下一步</div>
      </div>
      <el-dialog v-model="show_modal" :title="model_type" size="tiny">
        ...
        <el-form ref="form" :model="form" :rules="rule"  label-width="80px">
          ...
          <el-form-item label="上级部门" prop="parentId" v-if="!_disabled">
            <el-select v-model="form.parentId" placeholder="请选择" :disabled="_disabled">
              <el-option
