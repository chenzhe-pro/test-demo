<!--待办任务-->
<template>
  <div class="task todoTask ">
    <p class="list_title">待办任务</p>
    <div id="chart" style="height: 250px;"></div>
    <div class="todo_task_des">本周还有<span v-text="totalTask.outTime+totalTask.unover">2</span>项待办任务，<span v-text="totalTask.outTime">2</span>项任务已经过期</div>
...
//          status:'over'
    ...
    methods: {
      initChart:function () {
        ...
        _com.$http.post(common.projectPath + 'usr/job/week', data, {
          emulateJSON: false
        }).then(function (data) {
          data = data.body;
          if (data.code==='SUCCESS') {
            if(data.data)
            {
              var chart_data1=[],//延期任务
                chart_data2=[];//待完成任务
              for(var key in data.data)
              {
                chart_data1.push({x:parseInt(key)});//初始化散点的x轴,剩下的数据都往里面推
                chart_data2.push({x:parseInt(key)});//初始化散点的x轴,剩下的数据都往里面推
                if(!data.data[key])
                {
                  ...
                  {
                    _com.callback(data.data[key][j]);
                    var completeTimeStr=data.data[key][j].completeTime,
                      completeTimestamp=quicker.dateStringToLongTime(completeTimeStr,'-'),
                      hour_min_str=completeTimeStr.split(' ')[1],
                      hour_min_time=parseInt(hour_min_str.split(':')[0])+parseFloat(hour_min_str.split(':')[1]/60)+parseFloat(hour_min_str.split(':')[2]/3600);//把时分秒字符串转成小时数
                    if(data.data[key][j].status==='unover')//待完成任务
                    {
                      chart_data1.push({
                        x:parseInt(key),
                        y:parseFloat(hour_min_time),
                        completeTimeStr:completeTimeStr,
                        completeTimestamp:quicker.dateStringToLongTime(completeTimeStr,'-'),
                        description:data.data[key][j].description
                      });
                    }
                    else if(data.data[key][j].status==='delay')//延期任务
                    {
...
//                  tickInterval: 1000*3600,
                  tickPositions:[8,12,16,20,24],//写死刻度
                  labels: {
                    formatter:function(){//格式化刻度标签
...
//                      pointFormat: '{point.name} 完成时间:{point.y}',
                      pointFormatter:function () {//this就是point
                        var timeStr=quicker.longTimeToDateString(this.y,'-') ,str='完成时间:'+timeStr;
                        return '<b>'+this.description+'</b><br>'+'<div style="font-size: 12px;">完成时间:'+this.completeTimeStr+'</div>';
                      }
                    }
                  }
                },
                series: [{
                  name: '已延期',
                  color: 'rgba(210, 85, 101, .8)',
                  data:chart_data2

//                      {x:0,y:67},{x:5,y:55.6},{x:4,y:52.6},{x:1,y:55.6},{x:2,y:52.6},{x:3}
//              , [167.5, 59.0], [159.5, 49.2], [157.0, 63.0], [155.8, 53.6],
//              [170.0, 59.0], [159.1, 47.6], [166.0, 69.8], [176.2, 66.8], [160.2, 75.2]

                }, {
                  name: '待完成',
                  color: 'rgba(87, 167, 224, .8)',
                  data: chart_data1
//                    [0,65.6],[0,68.6],[2,65.6],[3,65.6],[4,65.6],[5,65.6],[6,65.6]
//              , [175.3, 71.8], [193.5, 80.7], [186.5, 72.6], [187.2, 78.8],
//              [181.5, 74.8], [184.0, 86.4], [184.5, 78.4], [175.0, 62.0], [184.0, 81.6]
      ...
      getMyTask: function () {
        ...
        _com.$http.post(common.projectPath + 'usr/job/weekJobCount', data, {
          emulateJSON: false
        }).then(function (data) {
          data = data.body;
          if (data.code==='SUCCESS') {
            _com.totalTask=data.data;
            if(data.data.pageDate&&data.data.pageDate.data) {
              for(var i=0;i<data.data.pageDate.data.length;i++){
                _com.callback(data.data.pageDate.data[i]);
              }
              _com.myTask=_com.myTask.concat(data.data.pageDate.data);
            ...
            if(_com.pagination(data.data.pageDate.page,data.data.pageDate.pageSize,data.data.pageDate.total))
            {
      ...
      callback:function (data) {
        var _com=this;
        //得到责任人和转发人
        data.liabilityMan=this.userInfo.realName;
        data.liabilityManId=this.userInfo.id;
        if(!data.jobEmployees) data.jobEmployees=[];
      ...
      update_sx:function () {
        ...
        {
//          delete this.form_sx.liabilityMan;
          url='usr/job/update';
...
//          this.form_sx.id=this.currentTask.id;
//          this.form_sx.jobEmployeeId=this.currentTask.jobEmployeeId;
        }
        else {
//          delete this.form_sx.content;
//          delete this.form_sx.status;
          url='usr/jobEmployee/save';
          form.contentType=this.form_sx.contentType;
          form.job={id:this.currentTask.id};
          form.employee={id:this.form_sx.liabilityMan};
//          this.form_sx.job=this.currentTask;
//          this.form_sx.employee={id:this.form_sx.liabilityMan};
        }
