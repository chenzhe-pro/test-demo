...
<template>
  ...
  <div class="wrapper stationSet manage">
    <!--<page-head></page-head>-->
    <div class="content_wrapper ">
      <!--<content_head :show-breadcrumb="false" show-menu="true"></content_head>-->
      <div class="content_content ">
        <el-tabs v-model="activeBox"  class="postManage">
          <el-tab-pane label="岗位管理" name="position">
            <div class="">
              <div class="set_content">
                <div class="left_content">
                  <p style="color:#4C5C6F;font-weight: bold;" class="f16 mb10">部门列表</p>
                    ...
                    <ul class="clearfix">
                      <li v-for="s in stationList">
                        <div class="card_node" :class="{stop_status:s.status==='N'}">
                          <div class="des">
                            <p><span class="node_label">岗位编号：</span><span class="node_value" v-text="s.number" style="margin-right: 90px;">1</span>
                              <span class="node_label" >岗位名称：</span><span class="node_value" v-text="s.name" style="margin-right: 90px;">总经理</span>
                              <span class="node_label" >上级岗位：</span><span class="node_value" style="margin-right: 90px;">{{s.parentName|null_to_text('无上级岗位')}}</span>
                              <span class="node_label">岗位序列：</span><span class="node_value" v-text="s.sequence"></span>
                            </p>
                            <p>
                              <span class="node_label">绩效周期：</span>
                              <span class="warn_tag mt5 mr10" v-for="p in s.postionJxPeriods" v-text="p.period">上级分解</span>
                          </span>
                              <span class="node_label" style="margin-left: 90px;">考核项目权重：</span><span class="node_value">KPI-{{s.kpiWeight}}% GS-{{s.gsWeight}}%</span>
                            </p>
                            <p>
                              <span class="node_label">职责标签：</span>
                              <span class="warn_tag mt5 mr10" style="background-color: #82C0EA;" v-for="y in s.labels" v-text="y.name">GS</span>
                          </span>
                              <span v-if="s.labels.length===0">暂无职责</span>
                              <span class="node_value" style="margin-left: 90px;">最近{{s.dayFix}}天日清表可以补填</span>
                            </p>
                            <p><span class="node_label">职责描述：</span>
                              <span class="node_value">{{s.remark|null_to_text('暂无职责描述')}}</span>
                            </p>
                          </div>
                          <div class="op p0" @click="edit(s.id)">
                            <a href="javascript:;" >编辑岗位信息</a>
                          </div>
                        </div>
                      </li>
                      <li class="add_node" v-if="currentPartment.id"><a href="javascript:;" @click="add"></a></li>
                    </ul>
...
//            dayFix:this.form.dayFix,
      ...
      getList:function (click) {
        var _com=this;
        _com.$http.get(common.projectPath+'usr/department/getDepartments',{
          params:{}
        }).then(function (data) {
          data=data.body;
          if(data.code=='SUCCESS'){
