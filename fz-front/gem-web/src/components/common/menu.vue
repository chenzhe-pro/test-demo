        ...
        <li v-for="x in roleData">
          <a href="javascript:;">
            ...
            <div class="t" >
              ...
              <p class="role_detail">{{x.remark|null_to_text('暂无角色描述')}}</p>
            </div>
          </a>
        </li>
      </ul>
...
<script>
  module.exports = {
    ...
    methods:{
      loadMenuDataByRole:function (donotMatch) {
        var menu=[],hide_menu=[],left_menu=[],right_menu=[],name='';
        ...
        if(menu.length>9)
        {
          ...
          {
            if(menu[j].url===this.check_menu.replace('/',''))
            {
              if(j>8)//菜单顺序在8以后
              {
                right_menu=menu.splice(j+1,menu.length-j);
                left_menu=menu.splice(0,j-8);
              }
              break;
            }
          }
        ...
        this.menuData=menu;
        if(left_menu.length===0)//菜单顺序8以内
          this.allData=this.menuData.concat(hide_menu);
        else
        {
          this.allData=left_menu.concat(this.menuData.concat(right_menu));
        }
        //做匹配操作
        if(!donotMatch)
        {
          if(!this.judgeMatch(this.allData,this.$route.fullPath))//当前页面不在该角色菜单中,自动跳转到该角色第一个菜单
          {
...
//          if(this.allData[i].url==url&&i>8){
//            var arr=arr.slice(i-8,i+1);
//            var st=this.allData.slice(0,i-8);
//              this.allData.splice(0,i-8);
//              this.allData=this.allData.concat(st)
//              this.menuData=arr;
//          }else{
////            this.check_menu=common.check_menu;
//          }
//        }
      ...
      changeRole:function (roleId,name) {
        ...
        this.loadMenuDataByRole(true);
        this.redirect_(this.allData[0].url);
//        this.roleModal=false;
        this.$message.success('角色已切换到'+name);
      },
      judgeMatch:function (menuData,url) {
        var isMatch=false;
        for(var i=0;i<menuData.length;i++)
        {
          if(menuData[i].url===url.replace('/',''))
          {
            isMatch=true;
            break;
          }
